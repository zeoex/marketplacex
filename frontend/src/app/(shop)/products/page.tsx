                  ...
                  {catList.filter((c: any) => !c.parentId).map((cat: any) => (
                    <button
                      ...
                      className={`w-full text-left text-sm px-3 py-2 rounded-lg transition-colors ${
                        filters.categoryId === cat.id ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700'
                      }`}
                    >
                ...
                <div className="space-y-1">
                  {CONDITIONS.map((c) => (
                    <button
                      ...
                      className={`w-full text-left text-sm px-3 py-2 rounded-lg transition-colors ${
                        filters.condition === c ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700'
                      }`}
                    >
                      {c.replace('_', ' ')}
                    </button>
                  ...
                  {Array.from({ length: meta.pages }, (_, i) => i + 1).map((p) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl font-medium text-sm transition-all ${
                        filters.page === p ? 'bg-primary-600 text-white' : 'bg-white dark:bg-slate-800 border border-slate-200 hover:border-primary-300'
                      }`}
                    >
                      {p}
                    </button>
